/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PostCategoryWhereInput } from "./PostCategoryWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PostCategoryOrderByInput } from "./PostCategoryOrderByInput";

@ArgsType()
class PostCategoryFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PostCategoryWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PostCategoryWhereInput, { nullable: true })
  @Type(() => PostCategoryWhereInput)
  where?: PostCategoryWhereInput;

  @ApiProperty({
    required: false,
    type: [PostCategoryOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PostCategoryOrderByInput], { nullable: true })
  @Type(() => PostCategoryOrderByInput)
  orderBy?: Array<PostCategoryOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PostCategoryFindManyArgs as PostCategoryFindManyArgs };
